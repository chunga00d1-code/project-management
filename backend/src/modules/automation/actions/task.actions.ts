...
export function createTaskActions(deps: { tasks: TaskActionPort; alerts: AlertPort }): ActionAdapter[] {
  const create: ActionAdapter = { type: "task.create", sensitive: false,
    validate(c) { validate(c, ["title"], ["title", "description", "priority", "labels", "projectId"]); if (typeof c.title !== "string") throw new Error("title must be a string"); },
  ...
  const update: ActionAdapter = { type: "task.update", sensitive: false,
    validate(c) { validate(c, ["taskId", "changes"], ["taskId", "changes"]); object(c.changes, "changes"); const safe = ["priority", "labels"]; if (Object.keys(c.changes).some(k => !safe.includes(k))) throw new Error("task.update only supports priority and labels"); },
  ...
  const assign: ActionAdapter = { type: "task.assign", sensitive: true,
    validate(c) { validate(c, ["taskId", "assignee"], ["taskId", "assignee"]); if (typeof c.assignee !== "string") throw new Error("assignee must be a string"); },
