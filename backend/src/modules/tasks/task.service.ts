...
import { nextTaskCode } from "../../core/counters.service.js";
...
export class TaskService {
  ...
  async findByCode(code: string) { return (await this.col()).findOne({ code: code.toUpperCase() }); }
  ...
  async setRelations(id: string, input: { dependencies?: string[]; watchers?: string[] }) { const now = new Date().toISOString(); await (await this.col()).updateOne({ _id: id }, { $set: { ...input, updatedAt: now } }); return (await this.col()).findOne({ _id: id }); }
  async linkPullRequest(taskId: string, input: { repository: string; number: number; status: TaskModel["status"]; description: string; prSyncStatus: "matched" | "mismatched"; prMismatchReasons: string[]; }) {
    return this.update(taskId, {
      repository: input.repository,
      pullRequestNumber: input.number,
      status: input.status,
      description: input.description,
      prSyncStatus: input.prSyncStatus,
      prMismatchReasons: input.prMismatchReasons,
    });
  }
}
