...
export const logger = winston.createLogger({ level: env.logLevel, defaultMeta: { service: "project-management", environment: env.nodeEnv }, format: winston.format.combine(redact(), winston.format.timestamp(), winston.format.errors({ stack: true }), winston.format.json()), transports });
