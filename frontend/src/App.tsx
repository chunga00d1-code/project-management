...
type Page = "tasks" | "projects" | "settings" | "users" | "operations";
const navItems: { id: Page; label: string; icon: string; permission?: "admin" | "users" }[] = [
  { id: "tasks", label: "Nhiệm vụ", icon: "▦" }, { id: "projects", label: "Dự án", icon: "◇" }, { id: "settings", label: "Cấu hình", icon: "⚙", permission: "admin" }, { id: "users", label: "Thành viên", icon: "♙", permission: "users" }, { id: "operations", label: "Vận hành", icon: "⌁", permission: "admin" },
];
export function App() {
  const [loggedIn, setLoggedIn] = useState(Boolean(localStorage.getItem("token"))); const user = (() => { try { return JSON.parse(localStorage.getItem("user") || "{}"); } catch { return {}; } })() as { email?: string; role?: string }; const [page, setPage] = useState<Page>("tasks"); const admin = hasPermission(user.role, "settings");
